...
const Navbar = ({ isCollapse, setCollapse }) => {
   ...
   const { name, image } = useSelector((state) => state.user.data);
   return (
      <>
         <nav className={`bg-white fixed top-0 right-0 h-screen-10 w-full ${isCollapse ? "md:w-95%" : "md:w-3/4 lg:w-4/5"} flex justify-between items-center px-4 shadow z-10`}>
            ...
            <div className="flex items-center gap-x-2">
               ...
               <div className="relative pb-2.5" onMouseEnter={() => setDropdown(!dropdown)} onMouseLeave={() => setDropdown(!dropdown)}>
                  <img src={require(`../../../../assets/images/avatars/${image}`)} alt={image} className="w-10 rounded-full object-cover object-center shadow-lg mt-2" />
